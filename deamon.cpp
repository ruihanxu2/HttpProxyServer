...
int search_cache(string request, int client_connection_fd, int thread_no) {
	...
	if (cache.count(request)) {
		...
		if (diff <= respond_in_cache.max_age || respond_in_cache.public_cache) {
			...
			fprintf(log_erss, "%d: Responding \"%s\"\n",thread_no, first_line_resp.c_str());
			fflush(log_erss);
...
void save_to_cache(string req_buffer_str, string cache_control_line,
	...
	if (cache_control_line == "public") {
		...
		fprintf(log_erss, "%d: cached, expires at %s", thread_no, asctime(timeinfo));
		fflush(log_erss);
		// print saved to cache...
	...
	fprintf(log_erss, "%d: not cacheable because cache-controll is not specified\n", thread_no);
	fflush(log_erss);
}
...
void handle_connect(int client_connection_fd, int socket_fd_to_server,
	...
	fflush(log_erss);fflush(log_erss);
	while (1) {
...
void handle_get(int client_connection_fd, int socket_fd_to_server,
	...
	fprintf(log_erss, "%d: Requesting \"%s\" from %s\n",thread_no, first_line.c_str() , http_request["Host"].c_str());
	fflush(log_erss);

	cout << "req_buffer sent\n";

	vector<char> resp_buffer(1024);
	int status = recv(socket_fd_to_server, &resp_buffer.data()[0], 1024, MSG_WAITALL);
	if (status == -1) {
	...
	for (; status == 1024; i++) {
		resp_buffer.resize(resp_buffer.size() + 1024);
		pos1 = (i + 1) * 1024;
		status = recv(socket_fd_to_server, &resp_buffer.data()[pos1], 1024, MSG_WAITALL);
		cout << status << endl;
	}
	cout << "----------------2\n";
	...
	fprintf(log_erss, "%d: Received \"%s\" from %s\n",thread_no, first_line_resp.c_str() , http_request["Host"].c_str());
	fflush(log_erss);
	// what I need: parse the cache-control line!
	string cache_control = parse_cache_control(resp_buffer);
...
void newConnection(int client_connection_fd, string request, char req_buffer[],
	...
	struct tm * timeinfo;
	timeinfo = gmtime(&now);
	fprintf(log_erss, "%d: %s from %s @ %s",thread_no, first_line.c_str(), hoststr, asctime(timeinfo));
	fflush(log_erss);
	...
	while (1) {
		...
		int status;
			...
			for (int i = 0;; i++) {
				...
				pool.enqueue(newConnection, client_connection_fd, request, req_buffer, i, hoststr);
