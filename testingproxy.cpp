...
int search_cache(string request, int client_connection_fd, int thread_no){
	...
	if(cache.count(request)){
		...
		if(diff<=respond_in_cache.max_age||respond_in_cache.public_cache){
			...
			int status = send(client_connection_fd, &resp_buffer.data()[0], resp_buffer.size(), 0);
...
void save_to_cache(string req_buffer_str, string cache_control_line, vector<char> resp_buffer, int thread_no){
...
void handle_get(int client_connection_fd, int socket_fd_to_server,
	...
	int status = recv(socket_fd_to_server, &resp_buffer.data()[0], 1024, 0);
	if (status == -1) {
		perror("error: ");
	}
...
//		status = recv(socket_fd_to_server, &resp_buffer.data()[1024 * i], 1024, 0);
//	}
//	
...
//	
//	
//	status = send(client_connection_fd, &resp_buffer.data()[0], resp_buffer.size(), 0);
//	if (status < 0) {
//		perror("write");
//	}
//	return;
	...
	for (; status == 1024; i++) {
		resp_buffer.resize(resp_buffer.size() + 1024);
		pos1 = (i + 1) * 1024;
		status = recv(socket_fd_to_server, &resp_buffer.data()[pos1], 1024, 0);
		cout<<status<<endl;
	}
	cout<<"----------------2\n";
	//what I need: parse the cache-control line!
	...
	cout<<"----------------4\n";
	status = send(client_connection_fd, &resp_buffer.data()[0], 1024*i + status, 0);
	cout<<"----------------5\n";
	if (status < 0) {
		perror("write");
	}
